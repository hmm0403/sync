...
int generate_file_list_name(file_list_t* fl)
{
	...
	fl->all_files = malloc(strlen(filename) + 1);
	if(!fl->all_files){
		return -1;
	}
	...
	fl->modified_files = malloc(strlen(filename) + 1);
	...
	fl->segment_files = malloc(strlen(filename) + 1);
	if(!fl->segment_files){
		return -1;
	}
	...
	fl->history = malloc(strlen(filename) + 1);
	if(!fl->history){
		return -1;
	}
	...
	fl->sync_fail = malloc(strlen(filename) + 1);
	if(!fl->history){
		return -1;
	}
